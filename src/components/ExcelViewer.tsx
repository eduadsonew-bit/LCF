    ...
    const loadExcel = async () => {
      try {
        ...
        const cleanedSheets = singleSheet.map(sheet => {
          ...
          // Insert "Domingo 26 de Abril" row after visual row 58 (index 57)
          if (newData.length > 57 && newData[57].length >= 1) {
            const totalCols = newData[57].length;
            const sundayRow: CellData[] = Array.from({ length: totalCols }, (_, i) => ({
              value: i === 0 ? 'DOMINGO 26 DE ABRIL' : null,
              style: {
                fill: '#2e7d32',
                font: { color: '#FFFF00', bold: true, size: 28, name: 'Arial' },
                alignment: { horizontal: 'center' as const, vertical: 'middle' as const },
            ...
            newData.splice(58, 0, sundayRow);
            if (sheet.rowHeights) {
              const row2Height = sheet.rowHeights[1] || 25;
              sheet.rowHeights.splice(58, 0, row2Height * 2);
            }
            sheet.rowCount = (sheet.rowCount || 0) + 1;
          }
