...
export default function ProgramacionPage() {
  ...
  // Convert base64 to blob URL for PDF files
  useEffect(() => {
    ...
    if (!selectedFile?.fileData || selectedFile.fileType !== 'pdf') {
      return;
    }
  ...
  }, [selectedFile?.id, selectedFile?.fileData, selectedFile?.fileType]);
  ...
  const isExcelFile = (file: ScheduleFile) => {
    return file.fileType === 'xlsx' || file.fileType === 'xls';
  };
      ...
      return (
        <div className="space-y-4">
          <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-green-800">Archivo Excel</p>
              <p className="text-sm text-green-600">El archivo se muestra a continuación</p>
            </div>
            <Button
              variant="outline"
              size="sm"
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge className={`${isExcel ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'} text-xs`}>
                        {isExcel ? 'Excel' : 'PDF'}
                      </Badge>
                    </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between flex-wrap gap-4">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${isExcelFile(selectedFile) ? 'bg-green-100' : 'bg-red-100'}`}>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        <Badge className={isExcelFile(selectedFile) ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}>
                          {isExcelFile(selectedFile) ? 'Excel' : 'PDF'}
                        </Badge>
